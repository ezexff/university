...
ebmpBYTE BMP::FindClosestColor(RGBApixel& input)
{
	...
	int* Red = Colors.Red;
	int ired = input.Red;
	int igreen = input.Green;
	int iblue = input.Blue;

	int *TempMatchArr;
	TempMatchArr = (int*)_mm_malloc(NumberOfColors * sizeof(int), 64);
#pragma omp simd aligned(TempMatchArr: 64)
	for (int i = 0; i < NumberOfColors; i++)
	{
		TempMatchArr[i] = (Red[i] - ired)*(Red[i] - ired) +
			(Green[i] - igreen)*(Green[i] - igreen) + (Blue[i] - iblue)*(Blue[i] -
				iblue);
	}
	for (int i = 0; i < NumberOfColors; i++)
	{
	if (TempMatchArr[i] < BestMatch)
	{
		BestI = (ebmpBYTE)i; BestMatch = TempMatchArr[i];
	}
	}
	_mm_free(TempMatchArr);
	...
	for ( i = 0; i<Width; i++)
		for ( j = 0; j< Height; j++)
		{
			...
			for (int ix = i - r; ix<i + r + 1; ix++)
				...
				{
					const int x = std::min(Width - 1, std::max(0, ix));
					const int y = std::min(Height - 1, std::max(0, iy));
					float tdsq = (ix - i)*(ix - i) + (iy - j)*(iy - j);
					float wght = exp(-dsq / (2 * r*r)) / (3.1415 * 2 * r*r); // расчет веса для соотвествующего пикселя окрестности
